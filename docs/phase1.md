> 本文记录历史 0.2 协议，不单独作为当前客户端实现依据。
> 当前需结合 [第二阶段协议](phase2.md) 的条件写入/反馈、[发送端标签](sender-tags.md) 的作用域，
> 以及 [完整性补全](completeness-review.md) 的单进程锁与授权撤销阅读。
> 当前快速开始见 [README](../README.md)。
